...
> Google **Antigravity CLI**（`agy`）を 10 個の typed MCP tool として
...
どこにインストールされたか、10 個の MCP tool が公開されたか、SKILL の
配置先、残っている任意項目。
````
...
- **MCP server**：`agymcp` が FastMCP stdio 経由で 10 個の typed JSON
